...
export default function Card(props: CardProps) {
    ...
    return (
      <div className={`artist-card w-40 h-44 pt-2 m-auto my-1 bg-zinc-800 rounded-md text-white`}>
        <div className="artist-card-holder relative text-center">
          ...
          <div className="artist-card-back bg-zinc-800 p-2">
            <Link href={"/shows"} className="show-link block mb-2 font-medium">{artistName}</Link>
            <p className="text-gray-300">{artistDescription}</p>
          </div>
        </div>
      </div>
    )
