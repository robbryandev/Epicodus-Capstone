...
export default function App({ Component, pageProps: { session, ...pageProps } }: AppProps) {
  ...
  const sendLogin = () => {
    if (renderRoutes.includes(router.asPath) && !session) {
      console.log("sent")
      router.push
    }
  }
  return (
    <SessionProvider session={session}>
      <div onLoad={() => session ? null : renderRoutes.includes(router.asPath) ? router.push("/") : null}></div>
      <div className={`app bg-background-main ${theme}`}>
        ...
        <div className={`settings bg-background-nav min-w-full ${renderRoutes.includes(router.asPath) ? "md:min-w-0" : ""} md:w-10 h-14 text-txt-main z-10 fixed top-0 pt-2 px-6 text-3xl`}>
          <Link href={renderRoutes.includes(router.asPath) ? "/home" : "/"} className='fixed top-1 left-3'>Local Shows</Link>
